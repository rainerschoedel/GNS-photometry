pro calibrate_ORIG, field, band, chip
...
magerr_si = 0.05 ; max acceptable SIRIUS magnitude uncertainty for photometric calibration stars
magerr_hawki = 0.05 ; max acceptable HAWK-I magnitude uncertainty for photometric calibration stars
...
; create model image to check whether scaled HAWK-I positions
...
; ;Now compute transformation with HAWK-I positions, but not the
...
; Create region of VVV alignment stars on VVV image
n_stars = n_elements(subc1)
...
dx_tmp = (x_vvv[subc1]-x_scaled[subc2])*0.34
dy_tmp = (y_vvv[subc1]-y_scaled[subc2])*0.34

set_plot,'PS',/interpolate
device, XOFFSET=0, YOFFSET=0, $
      FILENAME =  res_dir + 'position_uncertainty_pixels_' + strn(chip) + '.eps', XSIZE=20., YSIZE=12., $
...
astro = SOLVE_ASTRO(a_vvv[subc1], d_vvv[subc1], x[subc2]+1, y[subc2]+1, success=success, distort = 'none', reject = 2.0, NITER=3)

;x_shift = median(dx_tmp)/0.34
;y_shift = median(dy_tmp)/0.34
;astro = SOLVE_ASTRO(a_vvv[subc1], d_vvv[subc1], x[subc2]+x_shift, y[subc2]+y_shift, success=success, distort = 'none', reject = 2.0, NITER=3)

;astro = SOLVE_ASTRO(a_vvv[subc1], d_vvv[subc1], x[subc2], y[subc2], distort = 'tnx', etaorder=7, xiorder = 7);,n_tpvterms = 7, /NORTERMS)
print, astro
...
dra_tmp =  cos(d_vvv[subc1] * !PI/180.) * (a_vvv[subc1] - a[subc2]) * 3600.
ddec_tmp = (d_vvv[subc1] - d[subc2]) * 3600.

set_plot,'PS',/interpolate
device, XOFFSET=0, YOFFSET=0, $
      FILENAME =  res_dir + 'dpos_equatorial_' + strn(chip) + '.eps', XSIZE=20., YSIZE=12., $
      /portrait, /color,BITS_PER_PIXEL=8, encapsulated=1
!P.MULTI=[0,2,1]
!P.CHARSIZE=1.2
!P.THICK=4.0
...
dra_tmp =  cos(gbvvv[subc1] * !PI/180.) * (glvvv[subc1] - gl[subc2]) * 3600.
ddec_tmp = (gbvvv[subc1] - gb[subc2]) * 3600.

set_plot,'PS',/interpolate
device, XOFFSET=0, YOFFSET=0, $
      FILENAME =  res_dir + 'dpos_galactic_' + strn(chip) + '.eps', XSIZE=20., YSIZE=12., $
      /portrait, /color,BITS_PER_PIXEL=8, encapsulated=1
!P.MULTI=[0,2,1]
!P.CHARSIZE=1.2
!P.THICK=4.0
...
readcol, indir_list + star_list,  a, d, x, y, f, sx, sy, sf

; Read in SIRIUS photometric reference catalogue 
; -------------------------------------------------------

readcol, '/home/data/SIRIUS/list_all_Sirius.txt',$
  a_si, dec_si, J_si, dJ_si, H_si, dH_si, K_si, dK_si, Format = 'F,F,F,F,F,F,F,F'   

; check the uncertainties of SIRIUS to get an idea
...
writefits, res_dir + 'zpmap_' + band + '_chip' + strn(chip) +  '.fits', zpmap, /COMPRESS
writefits, res_dir + 'dzpmap_' + band + '_chip' + strn(chip) +  '.fits', dzpmap, /COMPRESS
...
;  !P.MULTI=[0,2,1]
  ...
  for j = 0, nbin - 1 do begin
    thisbin = where(abs(mag_ref_sirius - mags[j]) le 0.5, n_inbin)
    vals = dm[thisbin]
...
;fdens = f
;fdens[*] = 100
;im = image_model(x,y,fdens,xsize_hawki,ysize_hawki,'gaussian', dat)
;writefits, res_dir +'calibrated_stars_' + strn(chip) + '_density.fits', im, im_head, /COMPRESS
   

;Write calibrate list of stars to file
forprint, TEXTOUT= res_dir + 'stars_calibrated_' + band + '_chip' + strn(chip) + '.txt', a ,d, x, y, f, m, sx, sy, sf, sm, format='(10F16.6)', /NOCOMMENT
...
for iy = 0L, ys_map-1 do begin
  ...
  nearby = where(r le rmed, n_near)
  if (n_near gt 3) then begin
    resistant_mean, sm_map[nearby], 3.0, sm_mean, sm_sigma, rej, goodvec=good_cal
    smmap[ix,iy] = sm_mean
  endif
endfor
;print, ix
...
device, XOFFSET=0, YOFFSET=0, $
    ...
    cgplot, m, sm,  PSYM=4, Color = 'black', XTITLE='[' + band + strn(chip) +  ']', YTITLE='[d' + band + ']',$
      THICK = 0.5, XRANGE = [12,20], YRANGE = [0,0.3], SYMSIZE=0.2
device, /close 


; PLots of relative astrometric uncertainties
; =========================================
sx = sx * scale_holo * 1.e3
sy = sy * scale_holo * 1.e3
set_plot,'PS',/interpolate
device, XOFFSET=0, YOFFSET=0, $
    ...
    cgplot, m, sx,  PSYM=4, Color = 'black', XTITLE='[' + band + ', Chip ' + strn(chip) +  ']', YTITLE='dpos, x-axis [mas]',$
            THICK = 0.5, XRANGE = [11,20], YRANGE = [0,20], SYMSIZE=0.2
    device, /close
...
print, 'Uncertainties: '
RESISTANT_Mean,zpmap,5.0,zpmean,zpSigma,Num_Rej,goodvec = goodvec
print, 'ZP map robust mean and stddev: ' + strn(zpmean) + ', ' + strn(zpSigma*sqrt(n_elements(goodvec)-1))
print, 'Mean of ZP uncertainty map:  ' + strn(avg(dzpmap))
